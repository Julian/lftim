...
* [Definition of a group](https://en.wikipedia.org/wiki/Group_(mathematics)#Definition_and_illustration)
* [Definition of a partial order](https://en.wikipedia.org/wiki/Partially_ordered_set#Partial_order_relation)
...
There are three pieces of the short definition to decompose. Our structure will:
  ...
  * have a multiplication[^1], `*`, and this multiplication will satisfy the group axioms. Our Lean file will deal mostly with commutative groups (ones where a * b = b * a), so we will assume here that our group is commutative unless otherwise mentioned.
...
[^2] In general mathematical objects with 2 or more kinds of [structure](https://en.wikipedia.org/wiki/Algebraic_structure#Hybrid_structures) on them (here a multiplication and an order) seem to be "interesting" mostly when the two structures interact with each other somehow, rather than being completely unrelated, such as by simply asking that group elements be orderable without adding any additional axioms for how the multiplication interacts with the order.
...
From our lattice group's [docstring](https://leanprover-community.github.io/glossary.html#module-docstring), we can see that Lean has 3 [typeclasses](https://leanprover-community.github.io/glossary.html#class) which correspond quite directly to the above definition:
